'Title                         : Transaksi Document Import - BOLC
'Form                          : FrmBOLC
'Table Used                    : tbm_bank, tbm_bank_Reference, tbl_docimpr, tbl_budget
'Stored Procedure Used (MySQL) : SaveBOLC
'Form Created By               : Hanny, Oktober 2008
'Program Coding By             : Yanti, Oktober 2008

Public Class FrmBOLC
    Dim PO, BOLCNum As String
    Dim MyReader, MyReader2 As MySqlDataReader
    Dim num As Integer
    Dim TotalSisa As Double
    ...
    Sub New(ByVal pono As String, ByVal Currency As String, ByVal CurrName As String, ByVal TotalAmt As String, ByVal kurs As String, ByVal BOLC As String)
        ...
        If Trim(BOLC) <> "" Then
            ...
            If (btnPrint.Enabled) And (PunyaAkses("BO-P")) Then
        ...
        If Not MyReader Is Nothing Then
            While MyReader.Read
                Try
                    ...
                    TotalInvoice.Text = FormatNumber(MyReader.GetString("AMOUNT"), 2)
                    'btnReject.Enabled = (btnReject.Enabled = True) And (f_getenable(Status.Text))
                    'btnPrint.Enabled = (btnPrint.Enabled = True) And (f_getenable(Status.Text))
                Catch ex As Exception
                End Try
            End While
            ...
            Me.Text = "Budget Opening LC # " & Trim(BOLCNum) & " - Update"
    ...
    End Function
    Private Sub FrmBOLC_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim value = New System.Drawing.Point(55, 360)
        Dim strSQL, errMSg, mess As String
    ...
    Private Sub GetSisa()
        Dim strSQL, errMSg, mess As String
        Dim lv_lcamount As Double

        errMSg = "Failed when read BOLC data"
        strSQL = "SELECT t2.lcamount, (t1.poamount - t2.lcamount) sisa FROM " & _
                 "(SELECT t1.po_no, (SUM(t2.price * t2.quantity) * IF(t1.tolerable_delivery=0,1,(100+t1.tolerable_delivery)/100)) poamount FROM tbl_po t1, tbl_po_detail t2 " & _
                 "WHERE t1.po_no=t2.po_no AND t1.po_no='" & PO & "' GROUP BY t1.po_no) t1 " & _
                 "LEFT JOIN " & _
                 "(SELECT po_no, SUM(amount) lcamount FROM tbl_budget WHERE STATUS<>'Rejected' AND po_no='" & PO & "' GROUP BY po_no) t2 " & _
                 "ON t1.po_no=t2.po_no"

        'di ganti tanpa telorance
        strSQL = "SELECT t2.lcamount, (t1.poamount - t2.lcamount) sisa FROM " & _
                 "(SELECT t1.po_no, SUM(t2.price * t2.quantity) poamount FROM tbl_po t1, tbl_po_detail t2 " & _
                 "WHERE t1.po_no=t2.po_no AND t1.po_no='" & PO & "' GROUP BY t1.po_no) t1 " & _
                 "LEFT JOIN " & _
                 "(SELECT po_no, SUM(amount) lcamount FROM tbl_budget WHERE STATUS<>'Rejected' AND po_no='" & PO & "' GROUP BY po_no) t2 " & _
                 "ON t1.po_no=t2.po_no"
        ...
        If Not MyReader2 Is Nothing Then
            While MyReader2.Read
                Try
                    TotalSisa = MyReader2.GetString("sisa")
                    lv_lcamount = MyReader2.GetString("lcamount")
                Catch ex As Exception
                End Try
            End While
            CloseMyReader(MyReader2, UserData)
        End If
        If lv_lcamount > 0 Then
            If TotalSisa <= 0 Then
                mess = "Amount of PO " & PO & " already has BOLC document," & Chr(13) & Chr(10) & "Can't create BOLC anymore"
                MsgBox(mess)
                Me.Close()
            Else
                TotalAmount.Text = FormatNumber(TotalSisa, 2)
            End If
        End If
    End Sub
    ...
    Private Sub bank_data()
        Dim strSQL, errMSg, moreLess As String
        Dim v_tot, f_get_tol_del As Double

        '--hitung ulang saja jika Bank di ganti
        '--If BOLCNum > 0 Then
        '--'BOLC exist
        '--errMSg = "Failed when read BOLC data"
        ...
        '--         " where a.po_no = '" & PO & "' and a.ord_no=" & BOLCNum & " AND a.TYPE_CODE = 'BOLC' "
        '--Else
        'Create new
        ...
        moreLess = AmbilData("more_less", "tbm_bank", "bank_code='" & bank.Text & "' and currency_code='" & curr.Text & "'")
        MyReader2 = DBQueryMyReader(strSQL, MyConn, errMSg, UserData)
        If Not MyReader2 Is Nothing Then
            While MyReader2.Read
                Try
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            If btnSave.Text = "Save" Then
                SQLStr = "Run Stored Procedure SaveBOLC (Save," & PO & "," & OpeningDt & "," & bank.Text & "," & acno.Text & "," & str1 & "," _
                         & str2 & "," & str3 & "," & str4 & "," & remark.Text & "," & CTApp.Text & "," & AppDt & "," & CTFun.Text & "," & FinAppDt & ")"
                keyprocess = "Save"
            Else
                SQLStr = "Run Stored Procedure SaveBOLC (Updt," & PO & "," & OpeningDt & "," & bank.Text & "," & acno.Text & "," & str1 & "," _
                                         & str2 & "," & str3 & "," & str4 & "," & remark.Text & "," & CTApp.Text & "," & AppDt & "," & CTFun.Text & "," & FinAppDt & ")"
                keyprocess = "Updt"
            End If

            MyComm.CommandText = "SaveBOLC"
            ...
            If hasil = True Then
                f_msgbox_successful(btnSave.Text & " BOLC")
                btnClose_Click(sender, e)
            Else
                MsgBox(btnSave.Text & " BOLC failed'")
            End If
            'transaction.Commit()
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub btnReject_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReject.Click
        ...
        msg = "Reject Budget Opening LC #" & BOLCNum & " of " & PO & Chr(13) & Chr(10) & "Are you sure ?"
        If MsgBox(msg, MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            ...
            If affrow < 0 Then
                ...
                Exit Sub
            Else
                msg = "Budget Opening LC #" & BOLCNum & " of " & PO & " has been rejected"
                MsgBox(msg)
            End If
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Dim v_num As String
        Dim strSQL, errMsg, GetLC, GetPO As String
        Dim rows As Integer
        ...
        If Trim(lcno.Text) = "" Then
            strSQL = "SELECT lc_no, po_no FROM tbl_budget WHERE po_no='" & PO & "' AND ord_no='" & v_num & "' AND type_code='BOLC'"
        Else
            strSQL = "SELECT t2.company_code, t2.supplier_code, t1.bank_code, t1.lc_no, GROUP_CONCAT(DISTINCT t1.po_no SEPARATOR ', ') po_no FROM tbl_budget t1, tbl_po t2 " & _
                     "WHERE t1.po_no=t2.po_no AND t1.type_code='BOLC' AND t1.status <> ' Rejected' AND t1.lc_no= " & _
                     "(SELECT lc_no FROM tbl_budget WHERE po_no='" & PO & "' AND ord_no='" & v_num & "' AND type_code='BOLC') GROUP BY t2.company_code, t2.supplier_code, t1.bank_code, t1.lc_no"
        End If
        ...
        If Not MyReader2 Is Nothing Then
            While MyReader2.Read
                Try
                    rows = rows + 1
                    GetLC = MyReader2.GetString("lc_no")
                    GetPO = MyReader2.GetString("po_no")
                Catch ex As Exception
                End Try
            End While
            CloseMyReader(MyReader2, UserData)
            MsgBox("Creating Budget Opening LC " & GetLC & " of PO " & GetPO, MsgBoxStyle.Information, "Information")
        End If
        If rows > 1 Then
            MsgBox("Different Company, Supplier or Bank on this LC " & GetLC & "! ", MsgBoxStyle.Critical, "Warning")
            Exit Sub
        End If

        ViewerFrm.Tag = "BOLC" & v_num & PO
