...
''' Form                          : FrmBLRIL
''' Table Used                    : tbl_ril_quota, tbl_ril_quota_Detail, tbl_ril, tbl_ril_Detail, tbl_docimpr
''' Stored Procedure Used (MySQL) : SaveBLRILQ
''' Created By                    : AK, 10 November 2010
''' 
''' </summary>
''' <remarks></remarks>
...
Public Class FrmBLRILQ
    ...
    Sub New(ByVal Ship As String, ByVal POn As String, ByVal RIL As String, ByVal BL As String, ByVal Eksis As Boolean)
        ...
        If Trim(RIL) <> "" Then
            ...
            If (btnPrint.Enabled) And ((crtby.Text = UserData.UserCT) Or (PunyaAkses("CF-P"))) Then
                'If ((crtby.Text = UserData.UserCT) Or (PunyaAkses("CF-P"))) Then
        ...
        Else
            ...
            Me.Text = "BL Request Import Licence - Quota - New"
        End If
    End Sub
    Private Sub DisplayData(ByVal RIL As String)
        ...
        'strSQL = "select a.*,b.*, DATEDIFF(a.issueddt, a.submiteddt)+1 leadtime from tbl_ril_quota as a " & _
        '         "inner join tbl_ril_quota_detail as b on a.ril_no=b.ril_no " & _
        '         "where b.Ril_No='" & RILNum & "'"
        ...
        errMSg = "Failed when read RILQ data"
        ...
        If Not MyReader Is Nothing Then
            ...
            appcode.Text = temp1
            crtby.Text = temp2
            Me.Text = "BL Request Import Licence - Quota # " & Trim(RILNum) & " - Update"
        End If
    End Sub
    ...
    Private Function GetAppBy(ByVal fld As String) As String
        ...
        End Try
    End Function
    Private Sub FrmRILQ_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If RILNum = "" Then
            ...
            SQLStr1 = "sum(a.quantity) as 'RIL Quantity' "
            SQLStr2 = " "
            SQLStr3 = " "
        ...
        Grid.Columns(7).DefaultCellStyle.BackColor = Color.Gray
    ...
    Private Function CekData() As Boolean
        ...
        If RILNum = "" Then
            ...
            If totqty = 0 Then
                MsgBox("At least one of RILQ Quantity should be filled! ", MsgBoxStyle.Critical, "Warning")
                CekData = False
                Grid.Focus()
    ...
    Private Function GetDocAddress() As String
        strSQL = "select line_text from tbm_gov_address where doc_type='RQ' and group_code in ('" & cbDG.Text & "') group by line_no,line_text"
    ...
    Private Sub btnReject_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReject.Click
        ...
        msg = "Reject BL Request Import Licence - Quota #" & RILNum & " of " & BLNo & Chr(13) & Chr(10) & "Are you sure ?"
        If MsgBox(msg, MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            ...
            If affrow < 0 Then
                ...
                Exit Sub
            Else
                msg = "BL Request Import Licence - Quota #" & RILNum & " of " & BLNo & " has been rejected"
                MsgBox(msg)
            End If
    ...
    Private Function QtyOK(ByVal baris As Integer, ByVal Qty1 As Decimal, ByVal Qty2 As Decimal) As Boolean
        ...
        If Qty2 < 0 Then
            ...
            MsgBox("RILQ Quantity should be > 0")
            QtyOK = False
            Exit Function
        End If
        If (Qty2 > Qty1) Then
            ...
            Grid.Focus()
            Grid.CurrentCell = Grid(7, baris)
            MsgBox("Maximum Tolerable RILQ Quantity " & str)
            QtyOK = False
        End If
    End Function
    ...
    Private Sub Grid_DataError(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles Grid.DataError
        MsgBox("Invalid RILQ Quantity, input numeric value")
        DataError = True
    End Sub
    ...
    Private Sub BtnSrcDepTan_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSrcDepTan.Click
        Dim PilihanDlg As New DlgPilihan
        Dim strSQL, strsqlF, mess, DeptanNo As String
        Dim sisa As Decimal

        strSQL = "SELECT t1.ril_no RILNo, t1.deptan_no DeptanNo, t1.OpeningDt OpeningDate, t1.group_code DocumentGroup, t1.status Status, " & _
                             "t1.submiteddt SubmitedDate, t1.issueddt IssuedDate, " & _
                             "DATEDIFF(t1.issueddt, t1.submiteddt)+1 LeadTime, t1.remark Remark, t1.createdby CreatedId, m4.name CreatedBy, " & _
                             "t1.createddt CreatedDate, t1.approvedby ApprovedId, m5.name ApprovedBy, t1.approveddt ApprovedDate, " & _
                             "m1.company_name Company, m2.port_name DestinationPort, m3.port_name LoadPort, " & _
                             "(SELECT SUM(qtyAvailable) FROM vw_ril_quota_used WHERE vw_ril_quota_used.ril_no=t1.ril_no " & _
                             " AND material_code IN " & _
                             "  (SELECT material_code FROM tbl_shipping_detail WHERE shipment_no = " & ShipNo & ") " & _
                             ") QtyAvailable " & _
                             "FROM tbl_ril_quota t1, tbm_company m1, tbm_port m2, tbm_port m3, tbm_users m4, tbm_users m5 " & _
                             "WHERE t1.company_code = m1.company_code And t1.port_code = m2.port_code " & _
                             "AND t1.port_code = m3.port_code " & _
                             "AND t1.createdby = m4.user_ct AND t1.approvedby = m5.user_ct " & _
                             "AND t1.company_code = " & _
                             "  (SELECT DISTINCT sm1.company_code " & _
                             "   FROM tbl_ril_quota t1, tbl_shipping st1, tbm_plant sm1, tbl_shipping_detail st2 " & _
                             "   WHERE st1.plant_code = sm1.plant_code And st1.shipment_no = st2.shipment_no " & _
                             "AND st1.shipment_no = " & ShipNo & " AND st2.material_code IN " & _
                             "  (SELECT sst1.material_code " & _
                             "   FROM tbl_ril_quota t1, tbl_ril_quota_detail sst1 " & _
                             "   WHERE sst1.ril_no=t1.ril_no)) " 

        strsqlF = strSQL & "AND t1.ril_no LIKE  'FilterData1%' AND  t1.deptan_no LIKE  'FilterData2%' ORDER BY t1.issueddt DESC"
        strSQL = strSQL & "ORDER BY t1.issueddt DESC"

        PilihanDlg.Text = "Select Deptan No"
        ...
        If PilihanDlg.ShowDialog() = Windows.Forms.DialogResult.OK Then
            DeptanNo = PilihanDlg.DgvResult.CurrentRow.Cells.Item(1).Value.ToString
            sisa = PilihanDlg.DgvResult.CurrentRow.Cells.Item(18).Value.ToString
            If sisa <= 0 Then
                mess = "Deptan No. " & DeptanNo & " already been used," & Chr(13) & Chr(10) & "Can't create RIL based on Quota anymore"
                MsgBox(mess)
            Else

                no.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(0).Value.ToString
                tgl.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(2).Value.ToString
                cbDG.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(3).Value.ToString
                Status.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(4).Value.ToString

                txtDeptanNo.Text = DeptanNo
                dtSubmited.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(5).Value.ToString
                dtIssued.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(6).Value.ToString
                txtLeadtime.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(7).Value.ToString
                txtRemark.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(8).Value.ToString

                crtby.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(9).Value.ToString
                crtdate.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(11).Value.ToString
                appcode.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(12).Value.ToString
                appname.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(13).Value.ToString
                appdate.Text = PilihanDlg.DgvResult.CurrentRow.Cells.Item(14).Value.ToString
            End If
        End If
    End Sub

    Private Sub txtDeptanNo_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtDeptanNo.TextChanged
    ...
    Private Sub doc_text_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles doc_text.TextChanged

    End Sub

    Private Sub detail_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles detail.TextChanged
